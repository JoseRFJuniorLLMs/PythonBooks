...
# cmd history: redundant if readline built;
#
# use:  python psh.py
...
# - allow for multi-line statements- 
#   start == keyword and last == ':', {, ...
#############################################
...
##################################################                
# command for python or psh? check first word
##################################################
...
def ls(args): 
    if not args:
        ...
        system('ls ' + args[0])     # listdir(args[0])
...
def os(args):
    system(join(args))                      # ex: os cp shell.py a:
...
#
# loads '.PshInit' if present in cwd;
# should also check home dir too;  useful
