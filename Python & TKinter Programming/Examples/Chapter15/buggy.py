...
class Navigation:
    def __init__(self, master):
        ...
        for button, col in (('B1',1), ('B2',2), ('B3',3), ('B4',4)):
            setattr(self, '%s' % button, Button(frame, text=button,
                                                highlightthickness=2))
            getattr(self, '%s' % button).grid(padx=10, pady=6,
                                              row=0, col=col, sticky=NSEW)
        ...
        for button, col, action in (('Disable', 1, self.disable),
                                    ...
                                    ('Focus',   3, self.focus),):
            setattr(self, '%s' % button, Button(frame2, text=button,
                                    highlightthickness=2, command=action))
            getattr(self, '%s' % button).grid(padx=10, pady=6,
                                              row=0, col=col, sticky=NSEW)
