...
class Machine:
    ...
    def displaySet(self):
        ...
        for x,y, what in setDigits:
            value = eval(what)
            exec('self.canvas.create_text(%d,%d,text="%s",'
                 'font=("StingerLight", 36), '
                 'fill="#22AAFF", tags="settag")' % (x,y, value))
    ...
    def displayTime(self):
        ...
        for x,y, what in timeDigits:
            eval('self.canvas.create_text(%d, %d, text=%s,'
                 'font=("StingerLight", 36, "bold"),'
                 'fill="#FF6666", tags="timetag")' % (x,y, what))
        self.root.after(1000, self.displayTime)            
