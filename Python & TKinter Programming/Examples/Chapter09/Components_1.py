# Components_1.py for chapter 10 examples
...
class CardPuller(GUICommon):
	...
	bar1=Frame(self.puller_frame, width=width/8, height=8,
                   bg=self.dbase)
	bar1.place(relx=1.0, rely=[1.0,0][torb], anchor=[SE,NE][torb])
	bar2=Frame(self.puller_frame, width=width/3, height=24,
                   bg=self.vdbase)
	bar2.place(relx=1.0, rely=[0,1.0][torb], anchor=[NE,SE][torb])
        screw = Screw(self.puller_frame, diameter=10, base=self.base,
                      bg=self.lbase)
        screw.screw_frame.place(relx=0.3, rely=[0.2,0.8][torb],
                                anchor=CENTER)
